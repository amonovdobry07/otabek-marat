const translationsUz = {
    home: `Bosh sahifa`,
    products: `Tavarlar`,
    about: `Biz haqimizda`,
    contact: `Aloqa`,
    contactMe: "Bog'lanish uchun",
    ajoyibMebellar: "Zamonaviy Uy Dizayni",
    qoldaYasalgan: "Tabiatdan tug‘ilgan qulaylik",
    mahsulotlarniKorish: "Masulotlarni Ko'rish",
    bizningMahsulotlar: "Bizning Mahsulotlar",
    qulayYotoq: "Qulay Yotoq",
    yogochStol: "Yog‘och Stol",
    oshxonaNaborToplami: "Oshxona Nabor To‘plami",
    spalniy: "Spalniy",
    kiyimJavon: "Kiyim Javon",
    oshxonaNabori: "Oshxona Nabori",
    oshxonaMebel: "Oshxona Mebel",
    oshxonaNabor: "Oshxona Nabor",
    oshxonaDivani: "Oshxona Divani",
    som: "so'm",
    korish: "Ko‘rish",
    koproqKorish: "Ko‘proq ko‘rish",
    bizhaqimizda: "Biz haqimizda",
    bizZamonaviy: "Biz — zamonaviy mebel ishlab chiqaruvchi jamoamiz. Har bir mahsulotimizda nafislik, qulaylik va sifat uyg‘unligini saqlashga intilamiz. Mijozlarimiz uchun mebel — bu faqat buyum emas, balki uyda iliqlik va estetikani ifodalovchi elementdir",
    yildan: "2015-yildan buyon biz yuzlab oilalarga o‘z orzularidagi interyerni yaratishda yordam berib kelmoqdamiz. Sizning ishonchingiz bizning eng katta yutug‘imizdir.",
    yillikTajriba: "Yillik tajriba",
    mamnunMijozlar: "Mamnun mijozlar",
    dizaynLoyihalar: "Dizayn loyihalar",
    Bizbilanboglaning: "Biz bilan bog‘laning",
    Sizningfikringiz: "Sizning fikringiz biz uchun muhim! Quyidagi ma’lumotlar orqali bizga yozing yoki formani to‘ldirib yuboring — albatta javob beramiz.",
    Manzil: "Manzil",
    Telefon: "Telefon",
    Email: "Email",
    Ismingiz: "Ismingiz",
    Emailmanzilingiz: "Email manzilingiz",
    Xabaringizniyozing: "Xabaringizni yozing...",
    Yuborish: "Yuborish"
}




export default translationsUz